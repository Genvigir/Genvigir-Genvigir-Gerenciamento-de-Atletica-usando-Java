...
public class GerPessoas {
	...
	private static GerPessoas instance;
	...
	private List < Servidor > bdS = new ArrayList < Servidor > ();
        ...
        public List<Aluno> getBdA(){
            return bdA;
        }
        
        public List<ExAluno> getBdEx(){
            return bdEx;
        }
                
        public List<Servidor> getBdS(){
            return bdS;
        }
        
public Aluno CadAl(Aluno a) {
   
    for (Aluno aluno : bdA) {
        if (aluno.getCpf() == a.getCpf()) {
            return null;
        }
    }

    for (ExAluno exAluno : bdEx) {
        if (exAluno.getCpf() == a.getCpf()) {
            return null;
        }
    }

    for (Servidor servidor : bdS) {
        if (servidor.getCpf() == a.getCpf()) {
            return null;
        }
    }
...
public ExAluno CadExAluno(ExAluno ex) {
...
public Servidor CadServidor(Servidor s) {
...
//------------------------------------------------------------------------------------------------------------------------------------------------
	//Método Singleton
    	protected GerPessoas(){
	}
	
	//Método Singleton
	public static synchronized GerPessoas getInstance() {
        	if (instance == null) {
            		instance = new GerPessoas();
        }
        	return instance;
    	}
//------------------------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------------------------
public void removerPessoaPorCpf(int cpf) {
    // Procurando na lista de Alunos
    for (Aluno aluno : bdA) {
        if (aluno.getCpf() == cpf) {
            bdA.remove(aluno);
            return;
    ...
    // Procurando na lista de ExAlunos
    for (ExAluno exAluno : bdEx) {
        if (exAluno.getCpf() == cpf) {
            bdEx.remove(exAluno);
            return;
    ...
    // Procurando na lista de Servidores
    for (Servidor servidor : bdS) {
        if (servidor.getCpf() == cpf) {
            bdS.remove(servidor);
            return;
...
//------------------------------------------------------------------------------------------------------------------------------------------------
	public Aluno atualizaDadCpf(Aluno aluno){
            for(int i = 0; i < bdA.size(); i++){
                if(aluno.getCpf() == bdA.get(i).getCpf()){
                    String nome = JOptionPane.showInputDialog(null,"Informe o novo nome", "Atualiza Nome", JOptionPane.QUESTION_MESSAGE);
                    aluno.setNome(nome);
                    bdA.set(i, aluno);
                    return bdA.get(i);
                }
        ...
        public ExAluno atualizaDadCpf(ExAluno exAluno){
            for(int i = 0; i < bdEx.size(); i++){
                if(exAluno.getCpf() == bdEx.get(i).getCpf()){
                    String nome = JOptionPane.showInputDialog(null,"Informe o novo nome", "Atualiza Nome", JOptionPane.QUESTION_MESSAGE);
                    exAluno.setNome(nome);
                    bdEx.set(i, exAluno);
                    return bdEx.get(i);
                }
            }
        ...
        public Servidor atualizaDadCpf(Servidor servidor){
            for(int i = 0; i < bdS.size(); i++){
                if(servidor.getCpf() == bdS.get(i).getCpf()){
                    String nome = JOptionPane.showInputDialog(null,"Informe o novo nome", "Atualiza Nome", JOptionPane.QUESTION_MESSAGE);
	 ...
	 public Aluno entradaDadosAluno() {
		...
		try {
			...
			if(nome == null || nome.trim().isEmpty() || nome.matches("\\d+")) {
				throw new EntradaInvalidaException("Todos os campos devem ser preenchidos.");
			}
			a.setNome(nome);
